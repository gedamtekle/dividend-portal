...
var ICON='<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3h18v4H3zM5 7v14h14V7M9 11h6"/></svg>';
function screenHtml(){return '<div class="wrap" style="max-width:860px"><div class="h-eyebrow">Admin</div><h1 style="font-size:24px;font-weight:800;color:#0E1A2B;margin:2px 0 4px">Re-order Catalog</h1><p class="mut" style="margin-bottom:12px">Items clients can order. Paste each item\u2019s Stripe <b>Price ID</b> (price_...) to enable checkout; leave blank to hide the buy button.</p><div class="card pad" style="margin-bottom:14px"><div class="h-eyebrow">Add item</div><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:8px"><input id="ca-name" class="field" style="flex:1;min-width:160px" placeholder="Name"><input id="ca-cat" class="field" style="width:130px" placeholder="Category"><input id="ca-price" class="field" style="width:120px" placeholder="$ display"></div><input id="ca-desc" class="field" style="width:100%;margin-top:8px" placeholder="Description"><input id="ca-stripe" class="field" style="width:100%;margin-top:8px" placeholder="Stripe Price ID (price_...)"><div style="margin-top:8px"><button id="ca-add" class="btn primary">Add item</button></div></div><div id="ca-list"></div></div>';}
async function load(sec){var s=await ensureSb();var r=await s.from('reorder_catalog').select('*').order('sort');var rows=r.data||[];sec.querySelector('#ca-list').innerHTML='<div class="card" style="padding:0;overflow:hidden">'+(rows.length?rows.map(function(x){var linked=x.stripe_price_id?'<span class="pill" style="background:#1a7f4b11;color:#1a7f4b;font-size:10.5px">Stripe linked</span>':'<span class="pill mut" style="font-size:10.5px">no price</span>';return '<div style="border-bottom:1px solid #EEF0F4;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;gap:10px"><div><b>'+esc(x.name)+'</b> <span class="small mut">'+esc(x.category||'')+'</span> '+linked+(x.active?'':' <span class="pill mut" style="font-size:10.5px">hidden</span>')+'<div class="small mut">'+esc(x.description||'')+'</div></div><div style="display:flex;gap:8px;align-items:center"><input class="field ca-sp" data-id="'+x.id+'" style="width:150px;font-size:12px" placeholder="price_..." value="'+esc(x.stripe_price_id||'')+'"><button class="btn ca-save" data-id="'+x.id+'" style="background:#eef1f6;color:#0E1A2B;font-size:12px">Save</button><a href="#" data-del="'+x.id+'" class="small" style="color:#c0392b">Delete</a></div></div>';}).join(''):'<div style="padding:16px" class="small mut">No items.</div>')+'</div>';var list=sec.querySelector('#ca-list');if(!list.__w){list.__w=true;list.addEventListener('click',async function(e){var save=e.target.closest&&e.target.closest('.ca-save');var del=e.target.closest&&e.target.closest('[data-del]');var s2=await ensureSb();if(save){var id=save.getAttribute('data-id');var inp=list.querySelector('.ca-sp[data-id="'+id+'"]');var r2=await s2.from('reorder_catalog').update({stripe_price_id:(inp.value||'').trim()||null}).eq('id',id).select('id');toast(r2.error?'Couldn\u2019t save':'Saved');return;}if(del){e.preventDefault();if(!window.confirm('Delete this item?'))return;var r3=await s2.from('reorder_catalog').delete().eq('id',del.getAttribute('data-del')).select('id');if(r3.error){toast('Couldn\u2019t delete');return;}toast('Deleted');load(sec);}});}}
async function add(sec){var s=await ensureSb();var name=(sec.querySelector('#ca-name').value||'').trim();if(!name){toast('Add a name');return;}var r=await s.from('reorder_catalog').insert({name:name,category:(sec.querySelector('#ca-cat').value||'').trim()||null,price_display:(sec.querySelector('#ca-price').value||'').trim()||null,description:(sec.querySelector('#ca-desc').value||'').trim()||null,stripe_price_id:(sec.querySelector('#ca-stripe').value||'').trim()||null,sort:99}).select('id');if(r.error){toast('Couldn\u2019t add — admin only');return;}toast('Added');['ca-name','ca-cat','ca-price','ca-desc','ca-stripe'].forEach(function(id){sec.querySelector('#'+id).value='';});load(sec);}
function activate(nav,sec){[].forEach.call(document.querySelectorAll('.screen'),function(s){s.classList.remove('active');});[].forEach.call(document.querySelectorAll('.nav'),function(n){n.classList.remove('active');});sec.classList.add('active');nav.classList.add('active');try{window.scrollTo(0,0);}catch(e){}load(sec);}
