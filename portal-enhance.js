...
function screenHtml(){return '<div class="wrap" style="max-width:640px"><div class="h-eyebrow">Equipment</div><h1 style="font-size:24px;font-weight:800;color:#0E1A2B;margin:2px 0 6px">Re-Orders</h1><p class="mut" style="margin-bottom:16px">Request more terminals, hardware, or supplies for a merchant.</p><div class="card pad"><div style="margin-bottom:12px"><label style="font-size:13px;font-weight:700">Merchant</label><input id="ro-merchant" class="field" style="width:100%" placeholder="Which merchant is this for?"></div><div style="margin-bottom:12px"><label style="font-size:13px;font-weight:700">What do you need?</label><input id="ro-items" class="field" style="width:100%" placeholder="e.g. 2x Clover Mini terminals, paper rolls"></div><div style="display:flex;gap:10px;margin-bottom:12px"><div style="flex:1"><label style="font-size:13px;font-weight:700">Quantity</label><input id="ro-qty" class="field" style="width:100%" placeholder="e.g. 2"></div></div><div style="margin-bottom:12px"><label style="font-size:13px;font-weight:700">Notes</label><textarea id="ro-notes" class="field" rows="2" style="width:100%"></textarea></div><div id="ro-msg" class="small" style="min-height:16px;margin-bottom:8px"></div><button id="ro-submit" class="btn primary" style="width:100%;justify-content:center">Submit request</button></div><div id="ro-list" style="margin-top:16px"></div></div>';}
async function load(sec){var s=await ensureSb();var r=await s.from('reorders').select('*').order('created_at',{ascending:false});var rows=(r.data||[]).map(function(x){return '<div style="border-top:1px solid #EEF0F4;padding:10px 0;display:flex;justify-content:space-between"><div><b>'+esc(x.merchant_name||'—')+'</b> <span class="small mut">'+esc(x.items||'')+'</span></div><span class="pill mut" style="font-size:11px">'+esc(x.status)+'</span></div>';}).join('');sec.querySelector('#ro-list').innerHTML=rows?('<div class="card pad"><div class="h-eyebrow">Your requests</div>'+rows+'</div>'):'';}
async function submit(sec){var msg=sec.querySelector('#ro-msg');var s=await ensureSb();var u=await s.auth.getUser();var uid=u.data.user&&u.data.user.id;if(!uid){msg.textContent='Sign in again';return;}var items=(sec.querySelector('#ro-items').value||'').trim();if(!items){msg.style.color='#c0392b';msg.textContent='Tell us what you need.';return;}var btn=sec.querySelector('#ro-submit');btn.disabled=true;btn.textContent='Submitting…';var r=await s.from('reorders').insert({client_id:uid,merchant_name:(sec.querySelector('#ro-merchant').value||'').trim(),items:items,quantity:(sec.querySelector('#ro-qty').value||'').trim(),notes:(sec.querySelector('#ro-notes').value||'').trim()}).select('id').single();btn.disabled=false;btn.textContent='Submit request';if(r.error){msg.style.color='#c0392b';msg.textContent='Couldn’t submit — try again.';return;}msg.style.color='#1a7f4b';msg.textContent='Request submitted.';sec.querySelector('#ro-items').value='';sec.querySelector('#ro-qty').value='';sec.querySelector('#ro-notes').value='';load(sec);}
function activate(nav,sec){[].forEach.call(document.querySelectorAll('.screen'),function(s){s.classList.remove('active');});[].forEach.call(document.querySelectorAll('.nav'),function(n){n.classList.remove('active');});sec.classList.add('active');nav.classList.add('active');try{window.scrollTo(0,0);}catch(e){}load(sec);}
function mount(){var anchor=document.querySelector('.nav[data-screen="settings"]');if(!anchor)return false;if(document.querySelector('.nav[data-screen="reorders"]'))return true;var side=anchor.parentElement;var sp=(document.querySelector('section.screen')||{}).parentElement;if(!side||!sp)return false;var nav=document.createElement('div');nav.className='nav';nav.setAttribute('data-screen','reorders');nav.innerHTML=ICON+'Re-Orders';side.insertBefore(nav,anchor);var sec=document.createElement('section');sec.className='screen';sec.id='reorders';sec.innerHTML=screenHtml();sp.appendChild(sec);nav.addEventListener('click',function(){activate(nav,sec);});sec.querySelector('#ro-submit').addEventListener('click',function(){submit(sec);});if(typeof window.show==='function'&&!window.show.__dsReorderW){var o=window.show;var w=function(x){var out=o.apply(this,arguments);if(x!=='reorders'){var el=document.getElementById('reorders');if(el)el.classList.remove('active');}return out;};w.__dsReorderW=true;window.show=w;}return true;}
